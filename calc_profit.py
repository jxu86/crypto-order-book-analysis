...
class analysis_tradelog():
    ...
    def cal_diff(self,diff = None):
        print('tradelog==>', self.tradelog.head())
        if self.tradelog.shape[0] == 0:
            print ('empth tradelog found')
            return diff
    ...
    def cal_daily_diff(self,diff = None):
        ...
        # print('==>',self.tradelog.loc[:,['timestamp']])
        self.tradelog['timestamp'] = self.tradelog['timestamp'].map(utils.utcstr_to_datetime)

        st = dt(2019,3,2,0,0)

        # print('==>>', self.tradelog['timestamp'])
        # tmp_df = self.tradelog[self.tradelog.timestamp]
        # print('==>',self.tradelog[self.tradelog.timestamp > st self.tradelog.timestamp < st])



        # print('min==>',self.tradelog[['timestamp']].min())
        # print('min==>',self.tradelog[['timestamp']].max())

        stime = self.tradelog[['timestamp']].min()[0].replace(hour=0, minute=0, second=0,microsecond=0)
        etime = self.tradelog[['timestamp']].max()[0].replace(hour=0,minute=0, second=0,microsecond=0)
        ctime = stime
        print('stime=>', stime)
        ...
        while True:
            print('ctime=>', ctime)
            # print('ctime type=>', type(ctime))
            # i += 1
            if ctime > etime:
                break
            tmp_df = self.tradelog[self.tradelog.timestamp>=ctime]
            tmp_df = tmp_df[tmp_df.timestamp<ctime+datetime.timedelta(days=1)]
            diff = None
            ...
            for n in symbol_name.split('-'):
                if n == symbol_name.split('-')[0]:
                    diff[n] = diff.get(n,0) + (tmp_df['direction'] * tmp_df['filled_size'].astype('float')).sum()
                else:
                    diff[n] = diff.get(n,0) + (-1 * tmp_df['direction'] * tmp_df['filled_notional'].astype('float')).sum()
            diff['datetime'] = ctime
            ret.append(diff)
        ...
        for n in diff.keys():
            ...
            if n.lower() == base:
                ...
                if pair_name in self.price:
                    print('pair_name1=>', pair_name)
                    profit += diff[n] * self.price[pair_name]
                    print('####self.price[pair_name]1=>', self.price[pair_name])
                else:
                    print('pair_name=>', pair_name)
                    self.price[pair_name] = get_ticker(pair_name)
                    profit += diff[n] * self.price[pair_name]
                    print('####self.price[pair_name]2=>', self.price[pair_name])
        return profit
...
if __name__ == '__main__':
    ...
    for symbol in subscribe_okex:#subscribe_okex:
        all_trade_log[symbol] = get_order_df(symbol,time = set_time)  #, begin_time =dt(2019,2,28,16,0)
    diff = None
    volume = 0
    
    for symbol in all_trade_log:
        # all_trade_log[symbol] = pd.read_csv('./data/'+symbol+'.csv')
        all_trade_log[symbol].to_csv('./data/'+symbol+'.csv')
        a = analysis_tradelog(all_trade_log[symbol],price = {})
        print('all_trade_log[symbol]=>',all_trade_log[symbol])
        # diff_list = a.cal_daily_diff(diff)
        diff = a.cal_diff(diff)

        # rp = 0
        # for d in diff_list:
        #     profit = a.cal_profit(d,base = base_for_cal_profit)
        #     # r = a.cal_diff(d)
        #     d['profit'] = profit
        #     print('profit=>', profit)
        #     rp += profit
        # pd.DataFrame.from_dict(diff_list).to_csv('./data/'+symbol+'.csv')
        # print('diff_list==>', pd.DataFrame.from_dict(diff_list).to_csv('./data/'+symbol+'.csv'))
        # print('rp==>', rp)
